...
#                    diagnostic SSH is reachable WITHOUT waiting on any model fetch. Acquisition
#                    is deferred: the harness runs /opt/omp-acquire over SSH before serving.
#  unset (default) : blocking pinned acquisition (/opt/omp-acquire, fail-closed) THEN exec the
#                    baked server. Byte-unchanged serve-after-weights; never serves without weights.
set -e
