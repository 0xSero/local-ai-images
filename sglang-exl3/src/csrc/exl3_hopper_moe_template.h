/*
 * aikido-exl3: EXL3 (ExLlamaV3 trellis format) grouped MoE GEMM for Hopper on the Marlin MoE kernel template.
 *
 * This file is vLLM v0.29.0 csrc/libtorch_stable/moe/marlin_moe_wna16/marlin_template.h (Apache-2.0,
 * Copyright (C) Marlin.2024 Elias Frantar, modified by Neural Magic and the vLLM project) with the same changes
 * as csrc/exl3_hopper_template.h (the dense kernel; its AIKIDO diff was applied to the MoE template with `patch`,
 * three hunks by hand), plus: svh is per expert ([num_experts, prob_n], advanced like Marlin's bias) and the
 * in-launch output transform covers the valid rows of the moe block. No cooperative input prologue here. Changes,
 * all marked "AIKIDO":
...
// AIKIDO: vendored flat in csrc/third_party/marlin (vLLM: libtorch_stable/quantization/marlin/...)
#include "marlin.cuh"
...
#ifndef AIKIDO_MOE_MINBLOCKS_M8
  #define AIKIDO_MOE_MINBLOCKS_M8 2  // __launch_bounds__ min blocks per SM for the 8-row (decode) family: 2 = 128 registers (no spills), two co-resident blocks (research/12 section 13.5)
#endif
#ifndef AIKIDO_SLOT_REDUCE
  #define AIKIDO_SLOT_REDUCE 1  // cross-block reduce: 1 per-block slots for the 8-row family, 2 also 16-row, 0 Marlin's chain
#endif
...
template <const vllm::ScalarTypeId a_type_id,  // A ScalarType id
          ...
          >
__global__ __launch_bounds__(threads, (m_block_size_8 ? AIKIDO_MOE_MINBLOCKS_M8 : 1)) void Marlin(
    const int4* __restrict__ A,  // fp16 input matrix of shape mxk
    const int4* __restrict__ B,  // 4bit quantized weight matrix of shape kxn
    int4* __restrict__ C,        // fp16 output buffer of shape mxn
    int4* __restrict__ C_tmp,    // fp32 tmp output buffer (for reduce)
    ...
    bool use_atomic_add,   // whether to use atomic add to reduce
    bool use_fp32_reduce,  // whether to use fp32 global reduce
    // AIKIDO: fused gate+up launch. B and C are concatenated along n per expert; every shard has its own
    // pre-rotated input slab (own suh): slab s starts a_shard_stride int4 after slab s-1 (slab = [rows, prob_k]).
    // A column slice starting at column c belongs to shard (c >= shard_end0) + (c >= shard_end1) + (c >= shard_end2).
  ...
  static_assert(a_type_id == vllm::kFloat16.id() && c_type_id == vllm::kFloat16.id() &&
                ...
                !is_zp_float && ((exl3_cb >= 0 && exl3_cb <= 2) || exl3_cb == AIKIDO_CB_MCG_LUT));
  constexpr bool exl3_k6 = b_type_id == vllm::kU8B128.id();
  // AIKIDO: EXL3 K = 3 (GLM-5.3 TR3 experts): tiles staged byte-exact (24 words each, [E, k/16, n/64, 4, 24]); every
  ...
  // AIKIDO cb 13: the first 128 KB of dynamic shared memory hold the MCG table (filled from scales_ptr, which
  // carries the device LUT in that mode); everything of Marlin's starts after it.
  extern __shared__ int4 sh_raw[];
  constexpr bool exl3_lut = exl3_cb == AIKIDO_CB_MCG_LUT;
  constexpr int lut_int4 = exl3_lut ? 8192 : 0;
  int4* sh = sh_raw + lut_int4;
  const half* lut_sh = reinterpret_cast<const half*>(sh_raw);
  if constexpr (exl3_lut) {
    for (int i = threadIdx.x; i < 8192; i += threads) sh_raw[i] = scales_ptr[i];
    __syncthreads();
  }
  ...
  auto update_next_moe_block_data = [&]() {
    ...
    if (has_bias || (out_flags & 1)) {  // AIKIDO: svh [num_experts, prob_n] travels in b_bias_ptr
  ...
  // B sizes/strides (int4 units). AIKIDO K = 3: a k-tile row of n columns is n * 3 / 8 int4 (24 words per 16x16 tile).
  ...
  // Warp geometry (k split, reduction) in K = 4 units: identical for K = 4, K = 6 (2 vectors per lane) and K = 3.
  constexpr int b_sh_stride_threads = exl3_k3 ? thread_n_blocks * 8 : b_sh_stride / b_thread_vecs;
  constexpr int b_sh_stride_idx = exl3_k3 ? thread_n_blocks * 8 : b_sh_stride;   // K = 4-layout index base (b_sh_rd)
  ...
  constexpr int b_sh_cp_iters = div_ceil(b_sh_stage, b_sh_wr_delta);   // K = 3: predicated copy (stage not a multiple)
  ...
  if constexpr (exl3_k3) {
    b_gl_rd = 0;   // K = 3: thread-independent base; the copy loop adds (k-tile, column) itself
  ...
  b_sh_rd += b_sh_rd / b_sh_stride_idx * (b_sh_stride_idx * (b_sh_wr_iters - 1));
  // AIKIDO K = 3: this lane's two tile words and window shift (ExLlamaV3's constants, exl3_decode.cuh)
  ...
  constexpr int num_ints_per_thread = exl3_k3 ? 2 : 8 / pack_factor;   // AIKIDO K = 3: pack_factor 10 -> 0 (zp unused)
  int zp_sh_rd;
  if constexpr (has_zp) {
  ...
  [[maybe_unused]] uint32_t frag_b3[2][4][2];  // AIKIDO K = 3: (high, low) tile word per n-tile of the lane
  ...
  #pragma unroll
      for (int i = 0; i < a_sh_wr_iters; i++) {
        ...
        cp_async4_pred(&sh_a_stage[a_sh_wr_trans[i]], &A_sh[true_idx],
                       row < block_num_valid_tokens);
      }

      int4* sh_b_stage = sh_b + b_sh_stage * pipe;
      if constexpr (exl3_k3) {
        // AIKIDO K = 3: stage-linear int4 s = (k-tile kk, column col) of the slice's tile rows, byte-exact copy
  #pragma unroll
  ...
  #pragma unroll
    ...
    if constexpr (exl3_k3) {
      // AIKIDO K = 3: the K = 4-layout int4 index names (k-tile, n-group, lane); in the byte-exact layout that
  ...
  #pragma unroll
    for (int j = 0; j < 4; j++) {
      ...
      if constexpr (exl3_k3) {
        aikido_exl3::dq8_regs_3bits<FragB, exl3_cb>(frag_b3[k2][j][0], frag_b3[k2][j][1], k3_s2, frag_b0, frag_b1, lut_sh);
  ...
  #endif
        aikido_exl3::dq8_regs_4bits<FragB, exl3_cb>(aw, bw, frag_b0, frag_b1, lut_sh);
      }
  ...
  #pragma unroll
      for (int p = 0; p < div_ceil(had_items, had_warps); p++) {
        ...
        if (item < had_items && row < block_num_valid_tokens) {  // AIKIDO MoE: valid rows of this moe block
  ...
  #pragma unroll
      ...
      if (slice_count > 1 && !use_atomic_add) {
        ...
        // its running sum, adds, writes). Slot reduce (ported from the dense kernel, research/11 section 17): every
        // non-last block writes its own partial to its own C_tmp slot (region = block index; a block is non-last
        // only in the first slice of its stripe, and the blocks of a slice have consecutive indices, the last (top)
        // one being the lowest) and bumps the slice lock; the last block waits until all slice_count - 1 partials
        // are in and adds them in a fixed order (bottom block first): deterministic, nobody waits for a chain.
        // Compile-time family choice (a run-time choice costs ~1 us per launch, measured on the dense kernel):
        // AIKIDO_SLOT_REDUCE 1 = the 8-row family (decode), 2 = also the 16-row family, 0 = Marlin's chain.
        constexpr bool slot_family = AIKIDO_SLOT_REDUCE >= 2 ? thread_m_blocks == 1 : (AIKIDO_SLOT_REDUCE == 1 && m_block_size_8);
        if constexpr (slot_family) {
          static_assert(!is_a_8bit);
          if (!last) {
            barrier_acquire(&locks[locks_off], -2);  // no ordering condition (the lock is never -2): visibility only
